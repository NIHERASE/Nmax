...
RSpec.describe Nmax::StreamDigitScanner do
  describe 'Calls block for each number in stream "1 2_3.4"' do
    let(:stream) { StringIO.new('1 2_3.4') }
    subject { described_class.new(stream) }

    it do
      expect { |b| subject.scan(&b) }.to yield_successive_args('1', '2', '3', '4')
    end
  end

  describe 'Waits for the end of current number if it`s split '\
